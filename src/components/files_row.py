...
class FilesRow(tk.Frame):
    def __init__(self, receiver_address, progress_bar, mode_chooser, show_modal_func, decrypt_key_func, *args, **kwargs):
        super(FilesRow, self).__init__(*args, **kwargs)
        self.received_file = ReceivedFile(
            show_password_modal_func=show_modal_func, decrypt_key_func=decrypt_key_func, master=self)
